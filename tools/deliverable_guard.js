...
function scanExifMetadataForLeaks(deliverableDir, patterns, opts) {
  ...
  if (!fs.existsSync(exifExe)) { throw new Error("[FATAL_GATE_FAILURE] ExifTool missing: tools/exiftool/exiftool.exe"); }
  ...
  const r = spawnSync(exifExe, args, {
    ...
    const err = String((r && r.stderr) ? r.stderr : "").trim();
    throw new Error(
      "[FATAL_GATE_FAILURE] ExifTool failed (exit " + code + "). " +
      (err ? ("stderr: " + err.slice(0, 200)) : "No stderr.")
    );
  }

  const out = String(r.stdout || "");
  if (!out.trim()) { throw new Error("[FATAL_GATE_FAILURE] ExifTool produced empty output. Metadata audit is uncertain."); }

  let arr;
  try { arr = JSON.parse(out); } catch { throw new Error("[FATAL_GATE_FAILURE] ExifTool JSON parse failed. Metadata audit is uncertain."); }
  if (!Array.isArray(arr)) { throw new Error("[FATAL_GATE_FAILURE] ExifTool JSON was not an array. Metadata audit is uncertain."); }
...
function writeToolingHashes(caseRoot) {
  ...
  addHash("tools/manifest_validator.js", "tools/manifest_validator.js");
  addHash("tools/exiftool/exiftool.exe", "tools/exiftool/exiftool.exe");
  lines.push(`AF_DISABLE_COMPLETENESS_GATE: ${String(process.env.AF_DISABLE_COMPLETENESS_GATE || "").trim() === "1" ? "1" : "0"}`);const outPath = path.join(deliverableDir, "TOOLING_HASHES.txt");
