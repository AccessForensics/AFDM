...
// AF_COURT_SANITIZER_HELPER_V4_5_1
function escapeRegex(s) {
  // Prevent discovery of auditor environment metadata, conservative escaping for username token matching.
  return String(s).replace(/[.*+?^$()|[\]\\]/g, "\\$&");
}
...
function sanitizeForCourtLine(line) {
  ...
  // FULL PATH REDACTION, once a userdir anchor is detected, redact the entire absolute path sequence.
  // Preventing discovery of auditor environment metadata.
  s = s.replace(/\\\\\?\\[A-Za-z]:\\Users\\[^\r\n]*/gi, "[REDACTED_ABS_PATH]");
  s = s.replace(/[A-Za-z]:\\Users\\[^\r\n]*/gi, "[REDACTED_ABS_PATH]");
  s = s.replace(/\\Users\\[^\r\n]*/gi, "[REDACTED_ABS_PATH]");
  s = s.replace(/[A-Za-z]:\\Documents and Settings\\[^\r\n]*/gi, "[REDACTED_ABS_PATH]");
  ...
  // Unix/Mac (absolute home anchors only)
  s = s.replace(/\/Users\/[^\r\n]*/gi, "/[REDACTED_HOME]");
  s = s.replace(/\/home\/[^\r\n]*/gi, "/[REDACTED_HOME]");

  // file:// URIs
  s = s.replace(/file:\/\/\/[A-Za-z]:\/Users\/[^\r\n]*/gi, "file:///[REDACTED_ABS_PATH]");
  s = s.replace(/file:\/\/\/(Users|home)\/[^\r\n]*/gi, "file:///[REDACTED_HOME]");

  // Username token redaction (best effort)
  let uname = "";
  try { uname = (os.userInfo && os.userInfo().username) ? String(os.userInfo().username) : ""; } catch (e) { uname = ""; }
...
function assertNoCourtLeaks(text) {
  const t = String(text ?? "");
  const checks = [
    /\\\\\?\\[A-Za-z]:\\Users\\/i,
    /[A-Za-z]:\\Users\\/i,
  ...
  for (const re of checks) {
    if (re.test(t)) throw new Error("COURT_REDACTION_FAILURE: leak matched: " + re.toString());
  }
  let uname = "";
  try { uname = (os.userInfo && os.userInfo().username) ? String(os.userInfo().username) : ""; } catch (e) { uname = ""; }
  if (uname && new RegExp("\\b" + escapeRegex(uname) + "\\b", "i").test(t)) {
    throw new Error("COURT_REDACTION_FAILURE: username token leaked.");
  }
}

function writeNote(caseRoot, lines) {
  // AF_COURT_SANITIZE_WRITE_NOTE_V4_5_1
  if (Array.isArray(lines)) {
    const kill = ("tooling" + "_bundle" + "_path_abs").toLowerCase();
    lines = lines
      .filter(l => !String(l ?? "").toLowerCase().includes(kill))
      .map(sanitizeForCourtLine);
    assertNoCourtLeaks(lines.join("\n"));
  }
