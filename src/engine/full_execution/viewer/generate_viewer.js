 ...
 */
function generateViewer(packetDirPath, outputDir, accessOperatorId = "unknown_viewer") {
    ...
    // STRICT BOUNDARY ENFORCEMENT: Refuse to write inside the canonical packet.
    const resolvedPacketDir = path.resolve(packetDirPath);
    const resolvedOutputDir = path.resolve(outputDir);
    const relativeCheck = path.relative(resolvedPacketDir, resolvedOutputDir);

    // If the output dir is the same as packet dir, or a child of it, relative path doesn't start with ".."
    if (!relativeCheck.startsWith('..') && !path.isAbsolute(relativeCheck)) {
        throw new Error("Boundary violation: Viewer output directory cannot be inside the sealed canonical packet.");
    }
    ...
    // LOGGING: Explicitly log the view extraction outside the canonical boundary
    const logPath = path.join(resolvedOutputDir, matterId + "_view_access_log.txt");
    const logEntry = `[${new Date().toISOString()}] Viewer artifact generated by operator ${accessOperatorId}.\n`;
